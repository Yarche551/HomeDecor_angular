  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe((params) => {
      ...
      if (this.type) {
        ...
        if (params['types']) {
          this.activeParams.types = Array.isArray(params['types']) ? params['types'] : [];
        }

        // чекбоксы и аккордеоны уже открыты если выбрано что-то
        if (
  ...
  updateFilterParam(url: string, checked: boolean): void {
    if (this.activeParams.types && this.activeParams.types.length > 0) {
      ...
      if (existingTypeInParams && !checked) {
        this.activeParams.types = this.activeParams.types.filter((item) => item !== url); // удаление эл-та по url
